...
# 2.0.1 Changed Velero backup ttl from 7200h to 1440h (roughly 60 days)
...
get_DSOPPROD_App_auto () {
   ...
   echo -e  "${GREEN}$CNOW Auto backup completed for $APP${NC}" | tee -a ~/tmp/vback.log;
   # fortify
COMMAND='echo;export NO_PROXY=.vi2e.io; echo $NO_PROXY;getent hosts nessus-manager.vi2e.io;curl -ksq -o /dev/null -w %{http_code}\\n https://nessus-manager.vi2e.io:8834;curl -ksq -o /dev/null -w %{http_code}\\n https://10.10.99.91:8834;'
   CNOW=$(date +"%Y-%m-%d-%H%M")
  ...
  while true; do
    ...
    case $CHOICE in
      ...
      2 ) APP=confluence; NS=confluence; DBID=dsop-test-confluence-01;CNOW=$(date +"%Y-%m-%d-%H%M");echo -e  "${CYAN}$CNOW Manual backup started for $APP${NC}" | tee -a ~/tmp/vback.log; velero_rds_Backup;
          CNOW=$(date +"%Y-%m-%d-%H%M");echo -e  "${GREEN}$CNOW Manual backup completed for $APP${NC}" | tee -a ~/tmp/vback.log; break;;
      3 ) APP=fortify; NS=fortify; DBID=fortify-dsop-test;CNOW=$(date +"%Y-%m-%d-%H%M");echo -e  "${CYAN}$CNOW Manual backup started for $APP${NC}" | tee -a ~/tmp/vback.log; velero_rds_Backup;
          CNOW=$(date +"%Y-%m-%d-%H%M");echo -e  "${GREEN}$CNOW Manual backup completed for $APP${NC}" | tee -a ~/tmp/vback.log; break;;
...
velero_backup () {
  ...
  velero backup create $APP-$NOW-pvonly --include-resources pvc,pv --include-namespaces $NS --ttl 1440h0m0s --wait
...
gitlabBackup () {
  ...
  if [[ "$FULLAUTO" == "1" ]]; then
    echo -e "${YELLOW}Backing up $UCAPP${NC}"
    sudo cp /home/git/sipr-mission-bootstrap/bigbang/core/envs/dsop-test/patch-bigbang.yaml /home/maintuser/tmp/patch-bigbang-$NOW.yaml
...
echo
aws rds describe-db-snapshots --query 'DBSnapshots[].DBSnapshotIdentifier' | grep $(date +"%Y-%m-%d") | grep $CLUSTER
