...
# v1.4.0
# Removed Automated RDS Database Backups since Konvoy Role no longer allows it
# Making comment to do it manually
...
rds_backup () {
  NOW=$(date +"%Y-%m-%d-%H%M")
  echo
  echo -e "${YELLOW}Perform manual backup of RDS Database before continuing${NC}"
  pause
...
jiraBackup () {
  ...
  if [[ "${OPTION}" == "scaleup"  ]]; then
    flux suspend hr -n jira
    flux resume hr -n bigbang jira
  fi
  return 0
}
...
twistlockBackup () {
  ...
  kubectl get all -n $NS
  if [[ "${OPTION}" == "scaledown"  ]]; then
    kubectl delete jobs -n twistlock --all
    sudo su - git -c "kubectl delete -f /home/git/gitlab/defender.yaml"
    scale_down_deploy_and_stateful
  fi
  if [[ "${OPTION}" == "backup"  ]]; then
    echo -e "${YELLOW}Backing up $UCAPP${NC}"
  ...
  if [[ "${OPTION}" == "scaleup"  ]]; then
    scale_up_deploy
    flux resume hr -n bigbang $NS
    sudo su - git -c "kubectl apply -f /home/git/gitlab/defender.yaml"
  fi
  return 0
}
